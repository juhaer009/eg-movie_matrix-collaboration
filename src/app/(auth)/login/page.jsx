...
const LoginPage = () => {
  const router = useRouter();
  const { GoogleSignIN, signInUser, setUser, user } = useAuth(); 
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result.token) {
        ...
        // Update user state
        if (setUser) setUser({ email: formData.email, role });
  ...
  const RegWithGoogle = async () => {
    try {
      ...
      if (data.token) {
        ...
        if (setUser) setUser({ email: user.email, role });
      ...
      console.error("Google Login Error:", error);
      if (error.code === 'auth/popup-closed-by-user') window.location.reload();
      else setError("Social login failed. Please try again.");
    }
  };
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <div className="flex flex-1 flex-col justify-center px-4 py-12 sm:px-6 lg:flex-none lg:px-20 xl:px-32">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="mx-auto w-full max-w-sm lg:w-96">
          <div className="mb-10">
            <h2 className="text-3xl font-bold tracking-tight text-slate-900">Sign In</h2>
            <p className="mt-2 text-sm text-slate-500">Please enter your details to continue.</p>
          </div>
          ...
          <div className="space-y-6">
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Mail className="absolute left-3 top-3 h-4 w-4 text-slate-900" />
                  <Input id="email" type="email" placeholder="name@company.com" value={formData.email} onChange={handleChange} className="pl-10 h-11 text-slate-900 focus-visible:ring-indigo-500" required />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label className='text-slate-900' htmlFor="password">Password</Label>
                  <Link href="/forgot-password" size="sm" className="text-xs font-medium text-indigo-600 hover:text-indigo-500">Forgot password?</Link>
                </div>
                <div className="relative">
                  <KeyRound className="absolute left-3 top-3 h-4 w-4 text-slate-900" />
                  <Input id="password" type="password" placeholder="••••••••" value={formData.password} onChange={handleChange} className="pl-10 h-11 text-slate-900 focus-visible:ring-indigo-500" required />
                </div>
              </div>

              <div className="flex items-center space-x-2">
                <Checkbox id="remember" className="border-slate-300 data-[state=checked]:bg-indigo-600" />
                <Label htmlFor="remember" className="text-sm font-normal text-slate-600 cursor-pointer">Remember me for 30 days</Label>
              </div>
          ...
          <p className="mt-10 text-center text-sm text-slate-500">
            Don&apos;t have an account? <Link href="/register" className="font-semibold text-indigo-600 hover:text-indigo-500 underline-offset-4 hover:underline">Create an account</Link>
          </p>
        </motion.div>
      </div>
    </div>
