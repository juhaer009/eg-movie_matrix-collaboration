...
useEffect(() => {
  ...
  const uploadToImageBB = async (imageFile) => {
    const apiKey =
      process.env.NEXT_PUBLIC_IMGBB_API_KEY || "YOUR_IMAGEBB_API_KEY";

    if (!apiKey || apiKey === "YOUR_IMAGEBB_API_KEY") {
      throw new Error(
        "ImageBB API Key is missing. Please add NEXT_PUBLIC_IMGBB_API_KEY to your .env.local file.",
      );
    }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const { email, password, firstName, lastName, image } = formData;

      // 1️⃣ upload image first
      let photoURL = "";
      if (image) {
        photoURL = await uploadToImageBB(image);
      }
          ...
          <div className="mt-10">
            <div className="space-y-6">
              <form onSubmit={handleRegister} className="space-y-5">
                ...
                <div className="space-y-2">
                  <Label className="text-slate-500" htmlFor="profileImage">
                    Profile Image
                  </Label>
                  <Input
                    id="profileImage"
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-slate-900 py-6 text-white hover:bg-slate-800 transition-all shadow-lg shadow-slate-200"
                >
